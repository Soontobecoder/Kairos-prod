...
export default {
  ...
  methods: {
    async update () {
      try {
        console.log(this.form.creator, 'ini si pembuat')
        const response = await this.$axios.put('job-consol/' + this.$route.params.id, this.form)
        if (response.status === 200) {
          const { data } = response.data
          console.log(data, 'ini berhasil ngecreate bro')
          this.$toast.global.app_success('Succesfully updated jo console')
    ...
    async getData () {
      try {
        ...
        if (response.status === 200) {
          ...
          this.form = data
          console.log(data, 'ini hasil awal')
        }
      } catch (error) {
        const response = error.response.data
        const message =
